...
public class MultiSelectPopupState {
   public static MultiSelectPopupState ACTIVE = null;
   public static final int MAX_VISIBLE_ITEMS = 10;
   public static final float ITEM_ROW_H = 16.0F;
   public final MultiSelectSetting setting;
   public final String title;
   public final List<BooleanSetting> options;
   ...
   public MultiSelectPopupState(
      MultiSelectSetting setting, String title, List<BooleanSetting> options, float triggerX, float triggerY, float triggerW, float triggerH
   ) {
   ...
   public static void open(MultiSelectSetting setting, String title, List<BooleanSetting> options, float tx, float ty, float tw, float th) {
      if (ACTIVE != null && ACTIVE.setting == setting) {
         ACTIVE.isClosing = true;
      } else {
         ACTIVE = new MultiSelectPopupState(setting, title, options, tx, ty, tw, th);
      }
   }
   ...
   public static float calculateSubWidth() {
      ...
      if (medFont != null) {
         for (BooleanSetting opt : ACTIVE.options) {
            float w = medFont.getWidth(opt.getName(), 7.5F);
            if (w > maxTextW) {
               maxTextW = w;
            }
         }
      ...
      return Math.max(maxTextW + (hasScroll ? 42.0F : 36.0F), 96.0F);
   }
         ...
         if (ACTIVE.anim <= 0.005F && ACTIVE.isClosing) {
            ...
            for (int i = 0; i < ACTIVE.options.size(); i++) {
               ...
               if (!(rY + 16.0F < contentTop - 2.0F) && !(rY > contentBottom + 2.0F)) {
                  ...
                  float hovA = ACTIVE.itemHoverAnims.getOrDefault(opt.getName(), 0.0F);
                  hovA = GuiUtils.animate(hovA, rHov ? 1.0F : 0.0F, 18.0F, deltaTime);
                  ACTIVE.itemHoverAnims.put(opt.getName(), hovA);
                  if (hovA > 0.01F) {
                     Builder.rectangle()
                  ...
                  if (isSel && semiboldFont != null) {
                     try {
                        Builder.text().font(semiboldFont).text(opt.getName()).size(7.5F).color(textCol).build().render(matrix, textStartX, rY + 4.5F);
                     } catch (Exception e) {
                        GuiUtils.renderTextSafely(matrix, opt.getName(), textStartX, rY + 4.5F, textCol, 7.5F);
                     }
                  } else if (medFont != null) {
                     try {
                        Builder.text().font(medFont).text(opt.getName()).size(7.5F).color(textCol).build().render(matrix, textStartX, rY + 4.5F);
                     } catch (Exception e) {
                        GuiUtils.renderTextSafely(matrix, opt.getName(), textStartX, rY + 4.5F, textCol, 7.5F);
                     }
                  } else {
